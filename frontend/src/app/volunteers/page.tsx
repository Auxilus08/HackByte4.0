...
import { fetcher, API_ROUTES, Volunteer } from "@/lib/api";
import { 
  Users,
  Search,
  CheckCircle,
...
export default function VolunteersPage() {
  const { data, isLoading } = useSWR<{ total: number; items: Volunteer[] }>(
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative">
        ...
        <div className="flex gap-4 w-full sm:w-auto relative z-10">
          <button className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl text-sm font-bold uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(16,185,129,0.4)] hover:shadow-[0_0_30px_rgba(16,185,129,0.6)] border border-emerald-400 relative overflow-hidden group">
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      {volunteer.wallet_address ? (
                        <span className="font-mono text-xs text-purple-400 bg-purple-500/5 px-3 py-1.5 rounded-md border border-purple-500/20 flex items-center gap-2 w-fit">
                    ...
                    <td className="px-6 py-5 text-right">
                      <button className="text-[10px] uppercase tracking-widest text-blue-400 font-bold bg-blue-500/10 hover:bg-blue-500/20 px-4 py-2 rounded-lg transition-colors border border-blue-500/20">CONFIG</button>
                    </td>
                  </motion.tr>
                ))
              )}
